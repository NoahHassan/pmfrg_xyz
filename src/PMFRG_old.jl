...
function addX!(Workspace, is::Integer, it::Integer, iu::Integer, nwpr::Integer, Props)
    ...
    flavTransf12 = (
        get_w(wpw1) * get_w(-wpw2) < 0,
        get_w(ns) * get_w(-wpw2) < 0,
        get_w(ns) * get_w(wpw1) < 0,
    )
    flavTransf34 = (
        get_w(-wmw3) * get_w(-wmw4) < 0,
        get_w(ns) * get_w(-wmw4) < 0,
        get_w(ns) * get_w(-wmw3) < 0,
    )
    ...
    for Rij = 1:Npairs
        ...
        for k_spl = 1:Nsum[Rij]
            ...
            V12 = @SVector [Vert(n, ki, ns, wpw1, -wpw2, flavTransf12) for n = 1:21]
            V34 = @SVector [Vert(n, kj, ns, -wmw3, -wmw4, flavTransf34) for n = 1:21]
...
function addY!(Workspace, is::Integer, it::Integer, iu::Integer, nwpr::Integer, Props)
    ...
    wpw1, wpw2, wpw3, wpw4, wmw1, wmw2, wmw3, wmw4 = mixedFrequencies(ns, nt, nu, nwpr)
    flavTransf13 = (
        get_w(nt) * get_w(wmw3) < 0,
        get_w(-wmw1) * get_w(wmw3) < 0,
        get_w(-wmw1) * get_w(nt) < 0,
    )
    flavTransf24 = (
        get_w(-nt) * get_w(-wpw4) < 0,
        get_w(wpw2) * get_w(-wpw4) < 0,
        get_w(wpw2) * get_w(-nt) < 0,
    )
    flavTransf31 = (
        get_w(nt) * get_w(-wmw1) < 0,
        get_w(wmw3) * get_w(-wmw1) < 0,
        get_w(wmw3) * get_w(nt) < 0,
    )
    flavTransf42 = (
        get_w(-nt) * get_w(wpw2) < 0,
        get_w(-wpw4) * get_w(wpw2) < 0,
        get_w(-wpw4) * get_w(-nt) < 0,
    )
    ...
    for Rij = 1:Npairs
        ...
        V13 = @SVector [Vert(n, Rij, -wmw1, nt, wmw3, flavTransf13) for n = 1:21]
        V24 = @SVector [Vert(n, Rij, wpw2, -nt, -wpw4, flavTransf24) for n = 1:21]
        V31 = @SVector [Vert(n, Rij, wmw3, nt, -wmw1, flavTransf31) for n = 1:21]
        V42 = @SVector [Vert(n, Rij, -wpw4, -nt, wpw2, flavTransf42) for n = 1:21]
...
function InitializeAt(Par, Gamma)
    ...
    println(getChi_x(State, 100, Par))
    println(getChi_y(State, 100, Par))
    println(getChi_z(State, 100, Par))
    ...
    for Rij = 1:Npairs
        ...
        for nK = -lenIntw_acc:lenIntw_acc-1
            ...
            for nK2 = -lenIntw_acc:lenIntw_acc-1
                ...
                flavTransform =
                    (get_w(npwpw2) * get_w(-w2mw) < 0, get_w(-w2mw) < 0, get_w(npwpw2) < 0)
